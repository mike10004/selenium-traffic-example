...
public abstract class TrafficEater {
    ...
    protected MitmManager createMitmManager(BrowserMobProxy proxy) {
        MitmManager mitmManager = ImpersonatingMitmManager.builder()
                .rootCertificateSource(new KeyStoreFileCertificateSource(
                        CustomCertificate.KEYSTORE_TYPE,
    ...
    static class ProxyRequestFilters extends HttpFiltersAdapter {
        ...
        public ProxyRequestFilters(HttpRequest originalRequest, Iterable<String> headersToRemove, Iterable<? extends Map.Entry<String, ?>> headersToReplace, Iterable<? extends Map.Entry<String, ?>> headersToAdd) {
        ...
        public io.netty.handler.codec.http.HttpResponse proxyToServerRequest(io.netty.handler.codec.http.HttpObject httpObject) {
            if (httpObject instanceof io.netty.handler.codec.http.HttpRequest) {
                ...
                ((io.netty.handler.codec.http.HttpRequest) httpObject);
    ...
    static class EaterFiltersSource extends HttpFiltersSourceAdapter {
        ...
        private static final ImmutableSet<String> headersToRemove = ImmutableSet.of(HttpHeaders.Names.VIA);
        private static final ImmutableSet<? extends Map.Entry<String, ?>> headersToReplace = ImmutableSet.of();
        private static final ImmutableSet<? extends Map.Entry<String, ?>> headersToAdd = ImmutableSet.of(Pair.of("X-Traffic-Eater", "hungry"));

        @Override
        public HttpFilters filterRequest(io.netty.handler.codec.http.HttpRequest originalRequest) {
            return new ProxyRequestFilters(originalRequest, headersToRemove, headersToReplace, headersToAdd);
        }
    }
}
